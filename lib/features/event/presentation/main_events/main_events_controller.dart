...
import 'package:live_events_clean_architecture/core/app_global_services/app_secure_storage.dart';
...
class MainEventsController extends BaseController {
  ...
  final eventsRepository = Get.put<AbstractEventsRepository>(EventsRepositoryImp());
  ...
  Future<void> searchEvents({required String value}) async {
    updateState = LoadingState.waiting;
     ...
     final searchRequestModel= EventSearchRequestModel(page: currentResultPage, query: value);
  ...
  // await readUserLikes();
   if(favoriteList.isNotEmpty) {
     settingService.appSecureStorage.saveItemInStorage(
  ...
  Future<void> readUserLikes()async {
    String? result= await  settingService.appSecureStorage.readItemFromStorage(key: AppStorageKeys.favoriteList);
    if(result !=null){
      //favoriteList.clear();
