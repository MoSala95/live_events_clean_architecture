...
import '../ui_helpers/ui_styles.dart';
...
class CustomButton extends StatelessWidget {
  ...
  Widget _buildButton() {
    return _buildButtonContainer(
        decoration: BoxDecoration(
            color: color,
            border: Border.all(color: borderColor ?? AppColors.primaryColor.withOpacity(0.3), width: 1),
            borderRadius: borderRadius ?? BorderRadius.circular(10.r),
            boxShadow: const [
              BoxShadow(color: Color(0xb38b8b8b), offset: Offset(0, 2), blurRadius: 8, spreadRadius: 0)
            ],
            gradient: color == null
                ? gradientColor ?? UiStyles.blueGradient
                : null),
  ...
  _buildDimmedButton() {
    return _buildButtonContainer(
      decoration: BoxDecoration(
          borderRadius: borderRadius ?? BorderRadius.circular(10.r), color: AppColors.primaryColor.withOpacity(0.5)),
      child: _buildButtonBody(),
    );
  }

  Widget _buildButtonContainer({required Widget child, required Decoration decoration}) {
    return Container(
